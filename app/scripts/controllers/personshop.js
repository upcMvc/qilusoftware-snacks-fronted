...
angular.module('frontedApp')
  .controller('PersonshopCtrl', function ($scope,$http,$routeParams) {
    $http.get("http://mvc.y1code.cn:8080/shop/showshop?id="+$routeParams.id).then(function(data){

    });

    $http.get("http://mvc.y1code.cn:8080/goods/show?shopid="+$routeParams.id).then(function(data){
    ...
    $scope.put=function (index) {
      ...
      $http.get("http://mvc.y1code.cn:8080/goodlist/create?goodlist="+$scope.goods[index].id+"&num="+$scope.goods[index].quantity+"&name="+$scope.goods[index].name+"&price="+$scope.goods[index].price).then(function (data) {
